...
# Trial run with a smaller dataset

df = {'col_1': [4.5,2,3],
        'col_2': [4,2,3],
         'col_3': [5,1,4], 'col_4': [3,4,2], 'col_5': [1,5,5]
      }
df = pd.DataFrame(df)

user = {'col_1': [5],
        'col_2': [5],
         'col_3': [0], 'col_4': [0], 'col_5': [0]}
user = pd.DataFrame(user)

rewards = []
for j in range(len(user)):
    for i in range(len(df)):
        rewards.append(user.iloc[j,user.columns.get_loc(df.iloc[i,:].idxmax(axis=1))]/np.max(user.iloc[j,:]))
        
        df.drop(df.iloc[i,:].idxmax(axis=1),axis=1,inplace=True)
